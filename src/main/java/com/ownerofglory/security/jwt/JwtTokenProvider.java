package com.ownerofglory.security.jwt;
...
public class JwtTokenProvider {
    ...
    public String createSystemJwt() {
        Date nowDate = new Date();
        Date expirationDate = new Date(nowDate.getTime()
                + TimeUnit.SECONDS.toMillis(ApplicationConstants.SYSTEM_TOKEN_EXPIRATION_SEC));

        JwtParams params = JwtParams.builder()
                .id("0")
                .username(ApplicationConstants.SYSTEM_USERNAME)
                .authorities(ApplicationConstants.SYSTEM_AUTHORITY)
                .nowDate(nowDate)
                .expirationDate(expirationDate)
                .build();
